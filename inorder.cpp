...
void ineorder(Node*root)
{
    ...
    ineorder(root->left); // Traverse the left subtree  
    cout<< root->data << " "; // Visit the root node
    ineorder(root->right); // Traverse the right subtree
}
int main() {
    Node* root = BinaryTree(); // Create the binary tree
    cout << "Inorder traversal of the binary tree: ";
    ineorder(root); // Call the preorder function to display the tree
    return 0;
}   